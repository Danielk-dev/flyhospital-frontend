			...
			<main className="container my-5">
				...
				{store.loading && store.destinations.length === 0 ? (
                   ...
                   <ul className="nav nav-tabs">
							...
							<li className={`nav-link ${activeRegion === null ? 'active' : ''}`} onClick={() => setActiveRegion(null)}>All</li>
							{store.regions.map((region) => (
								<li key={region.id} className={`nav-link ${activeRegion === region.id ? 'active' : ''}`} onClick={() => setActiveRegion(region.id)}>
									{region.name}
								</li>
							))}
							</ul>
							...
							{filtered.map((destination) => (
								<div key={destination.id} className="col-md-3">
									<div className="card h-100">
										<img src={getImageUrl( destination.secondary_image)} className="card-img-top" alt={destination.country_name || destination.name} />
										<div className="card-body">
											...
											<h3>
												<Link href={`/hospitals?country_id=${destination.id}`} className="text-decoration-none">
												{destination.country_name || destination.name}</Link>
											</h3>
											 <span className="card-rating">
											<div className="hospital-count">Total Hospitals { destination.hospitals_count ?? 0 }</div>
										</span>
											</div>
										
											 <div className="mt-2">
												<a href={`/hospitals?country_id=${destination.id}`} className="btn btn-sm btn-primary">
