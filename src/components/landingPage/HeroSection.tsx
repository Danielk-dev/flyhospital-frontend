...
export default function HeroSection() {
	...
	useEffect(() => {
		...
		if (procedure.length === 0) loadprocedure();
	}, [countries.length, country_id, loadCountries, loadprocedure, procedure.length]);

	useEffect(() => {
		if (selectedCountryId) loadCities(selectedCountryId);
	}, [loadCities, selectedCountryId]);

	useEffect(() => {
		if (selectedCategoryId) loadSubprocedure(selectedCategoryId);
	}, [loadSubprocedure, selectedCategoryId]);

	const hasActiveFilters = treatment_id || category_id || country_id || city_id;

	const submitSearch = () => {
		const params = new URLSearchParams();
		if (search) params.set('search', search);
		if (country_id) params.set('country_id', String(country_id));
		if (city_id) params.set('city_id', String(city_id));
		if (category_id) params.set('category_id', String(category_id));
		if (treatment_id) params.set('treatment_id', String(treatment_id));
		router.push(`/hospitals?${params.toString()}`);
	};

	const applyTreatmentFilter = () => {
		setCategoryId(selectedCategoryId || null);
		setTreatmentId(selectedTreatmentId || null);
		setShowTreatmentModal(false);
		if (selectedTreatmentId) {
			const treatment = subprocedure.find((t) => String(t.id) === selectedTreatmentId) as { name?: string } | undefined;
	...
	const applyDestinationFilter = () => {
		setCountryId(selectedCountryId || null);
		setCityId(selectedCityId || null);
		...
		<section className="hero-section">
			<div className="hero-layout">
				<div className="hero-text">
					...
					<div className="search-container">
						...
						<div className="search-bar">
							<input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search hospitals by name..." />
							...
							<div className="active-filters">
								{treatment_id && (
									<div className="filter-chip">
										Procedure selected
										<button className="chip-close" onClick={() => setTreatmentId(null)}>×</button>
									</div>
								)}
								{(country_id || city_id) && (
									<div className="filter-chip">
										Destination selected
										<button className="chip-close" onClick={() => { setCountryId(null); setCityId(null); }}>×</button>
									</div>
								)}
							</div>
						)}
				...
				<div className="hero-image">
					<img src="/assets/img/banner.jpg" alt="Patients at a hospital reception" className={`rounded-left-bottom`} onLoad={() => setImageLoaded(true)} onError={() => setImageLoaded(true)} />
				</div>
			</div>
